/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   AForm.cpp                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: mattcarniel <mattcarniel@student.42.fr>    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/12 15:24:14 by mattcarniel       #+#    #+#             */
/*   Updated: 2026/05/15 15:06:21 by mattcarniel      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <iostream>

#include "AForm.hpp"
#include "Bureaucrat.hpp"

#define ANSI_RESET		"\033[0m"
#define ANSI_RED		"\033[31m"

#ifdef DEBUG

# define ANSI_BOLD		"\033[1m"


# define ANSI_MAGENTA	"\033[35m"
# define ANSI_BMAGENTA	"\033[95m"

# define LOG_DEBUG(x) \
	do { \
		std::cout	<< ANSI_BMAGENTA << ANSI_BOLD \
					<< "[DEBUG] " \
					<< ANSI_RESET << ANSI_MAGENTA \
					<< x \
					<< ANSI_RESET \
					<<std::endl; \
	} while (0)
#else
# define LOG_DEBUG(x) do {} while (0)
#endif

int	AForm::validateGrade(int grade)
{
	if (grade < GRADE_MAX)
		throw AForm::GradeTooHighException();
	if (grade > GRADE_MIN)
		throw AForm::GradeTooLowException();
	return (grade);
}

AForm::AForm(void) :
	_name("unknown"),
	_is_signed(false),
	_sign_grade(GRADE_MIN),
	_exec_grade(GRADE_MIN)
{	LOG_DEBUG("[AForm] Default constructor called");}

AForm::AForm(const std::string& name, int sign_grade, int exec_grade) :
	_name(name),
	_is_signed(false),
	_sign_grade(validateGrade(sign_grade)),
	_exec_grade(validateGrade(exec_grade))
{	LOG_DEBUG("[AForm] Parameterized constructor called");}

AForm::AForm(const AForm &other) :
	_name(other._name),
	_is_signed(other._is_signed),
	_sign_grade(other._sign_grade),
	_exec_grade(other._exec_grade)
{	LOG_DEBUG("[AForm] Copy constructor called");}

AForm::~AForm(void)
{	LOG_DEBUG("[AForm] Destructor called");}

AForm&	AForm::operator=(const AForm &other)
{
	LOG_DEBUG("[AForm] Assignment operator called");

	if (this != &other)
		this->_is_signed = other._is_signed;
	return (*this);
}

const std::string&	AForm::getName(void) const
{
	return (this->_name);
}

bool				AForm::isSigned(void) const
{
	return (this->_is_signed);
}

int					AForm::getGradeToSign(void) const
{
	return (this->_sign_grade);
}

int					AForm::getGradeToExecute(void) const
{
	return (this->_exec_grade);
}

int	AForm::getMaxGrade(void)
{
	return (GRADE_MAX);
}

int	AForm::getMinGrade(void)
{
	return (GRADE_MIN);
}

void	AForm::beSigned(const Bureaucrat& b)
{
	if (b.getGrade() > this->_sign_grade)
		throw AForm::GradeTooLowException();
	this->_is_signed = true;
}

void	AForm::execute(const Bureaucrat& e) const
{
	if (!this->_is_signed)
		throw AForm::FormNotSignedException();
	if (e.getGrade() > this->_exec_grade)
		throw AForm::GradeTooLowException();
}

const char*	AForm::GradeTooHighException::what() const throw()
{
	return (ANSI_RED " Form: grade too high !" ANSI_RESET);
}

const char*	AForm::GradeTooLowException::what() const throw()
{
	return (ANSI_RED " Form: grade too low !" ANSI_RESET);
}

const char*	AForm::FormNotSignedException::what() const throw()
{
	return (ANSI_RED " Form: Form is not signed !" ANSI_RESET);
}

std::ostream&	operator<<(std::ostream& os, const AForm& f)
{
	os << "AForm \"" << f.getName() << "\" "
	<< "[signed: " << (f.isSigned() ? "yes" : "no")
	<< ", sign grade: " << f.getGradeToSign()
	<< ", exec grade: " << f.getGradeToExecute() << "]";
	return (os);
}
