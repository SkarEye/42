...
/*   Updated: 2025/09/04 15:00:26 by macarnie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	draw_str(const char *str, t_pixel p, t_txt txt, t_fdf *fdf)
{
	...
	count = get_space_count(str, txt.size.x / (G_W * txt.scale));
	line_len = get_line_len(str, txt.size.x / (G_W * txt.scale));
	leftover = txt.size.x - line_len * G_H * txt.scale;
	i = 0;
	while (i < line_len)
	{
		if (str[i] == ' ' && txt.align == A_JUSTIFIED)
		{
			p.x += 	leftover / count;
...
void	draw_text(const char *str, t_pixel p, t_txt txt, t_fdf *fdf)
{
	...
	while (str[i])
	{
		...
		d.x = get_pixel_offset(&str[i], txt);
		d.y += G_H * txt.scale;
		if (d.y > txt.size.y)
			return ;
	}
